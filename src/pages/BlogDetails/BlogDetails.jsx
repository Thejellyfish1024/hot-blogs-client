...
const BlogDetails = () => {
    ...
    const { data, isLoading } = useQuery({
        queryKey: [`${id}`],
        queryFn: async () => {
            const data = await fetch(`https://hot-blogs-server.vercel.app/blogs/${id}`)
    ...
    const handleCommentSend = () => {
        ...
        fetch('https://hot-blogs-server.vercel.app/comments', {
            method: 'POST',
            headers: {
                    ...
                    <h2 className="font-bold text-2xl">Comments</h2>
                    {
                        user?.email === data?.email ?
                            ...
                            <div className="flex items-center gap-4 my-5">
                                ...
                                <div className="md:w-1/2 flex border border-blue-500 rounded-2xl ">
                                    <input required name="search" value={value} onInput={onInput} type="text" placeholder="Write your comment..." className=" rounded-l-2xl md:pl-5 pl-2 md:text-lg w-full " />
                                    <button onClick={handleCommentSend} className="bg-blue-500 rounded-r-2xl md:py-3 py-2 md:px-8 px-4 font-bold text-white">Send</button>
                                </div>
                            </div>
                    }
                    {/* others comments */}
